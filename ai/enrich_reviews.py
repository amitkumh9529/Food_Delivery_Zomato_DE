...
def create_output_table(cursor):
    cursor.execute("CREATE SCHEMA IF NOT EXISTS FD.AI")
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS FD.AI.REVIEW_ENRICHED (
            REVIEW_ID STRING,
...
def get_reviews_to_enrich(cursor):
    cursor.execute(f"""
        ...
        FROM FD.RAW.REVIEWS
        WHERE REVIEW_ID NOT IN (SELECT REVIEW_ID FROM FD.AI.REVIEW_ENRICHED)
        LIMIT {SAMPLE_N}
    """)
...
def save_results(cursor, results):
    ...
    cursor.executemany(
        """
        INSERT INTO FD.AI.REVIEW_ENRICHED
